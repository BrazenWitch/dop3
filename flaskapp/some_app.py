...
##открываем изображение
    ...
    result_img[:,:,0] = img[:,:,0]*redlist[0]
    result_img[:,:,1] = img[:,:,0]*redlist[1]
    result_img[:,:,2] = img[:,:,0]*redlist[2]

    green = green[4:-1]
    greenlist=green.split(',')
    list(map(float,greenlist))
    greenlist=np.array(greenlist).astype('float')/255.0
    result_img[:,:,0] = result_img[:,:,0]+img[:,:,1]*greenlist[0]
    result_img[:,:,1] = result_img[:,:,1]+img[:,:,1]*greenlist[1]
    result_img[:,:,2] = result_img[:,:,2]+img[:,:,1]*greenlist[2]

    blue = blue[4:-1]
    bluelist=blue.split(',')
    list(map(float,bluelist))
    bluelist=np.array(bluelist).astype('float')/255.0
    result_img[:,:,0] = result_img[:,:,0]+img[:,:,2]*bluelist[0]
    result_img[:,:,1] = result_img[:,:,1]+img[:,:,2]*bluelist[1]
    result_img[:,:,2] = result_img[:,:,2]+img[:,:,2]*bluelist[2]

    result_img[1.0<result_img]=1.0
    print(result_img)
